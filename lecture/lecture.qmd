...
Il est possibile de contrôler finement la jointure (points sur les frontières, ...) avec l'argument `join` qui par défaut est fixé à `st_intersects()`. Voir [ici](https://en.wikipedia.org/wiki/DE-9IM) pour uné définition précise des prédicats spatiaux possibles. 
...
Voir également [ici](https://github.com/r-spatial/sf/issues/474) et [là](https://stackoverflow.com/questions/45719790/create-voronoi-polygon-with-simple-feature-in-r).
...
Dans cette partie, nous allons visualiser des données d'[accidents corporels de la circulation routière](https://www.data.gouv.fr/fr/datasets/bases-de-donnees-annuelles-des-accidents-corporels-de-la-circulation-routiere-annees-de-2005-a-2019/) de 2019.
...
Il faut convertir ce `data.frame` en objet spatial (`sf`) grâce à la fonction `st_as_sf`. Il suffit de spécifier le nom des colonnes contenant les coordonnées ainsi que la projection, ici `CRS = 4326` (WGS 84). On transforme ensuite ces données en `CRS = 2154` (Lambert 93).
...
- Repris par Wickham, H. (2010). A layered grammar of graphics. Journal of Computational and Graphical Statistics, 19(1), 3-28. Et implémenté dans `ggplot2`
...
ggplot() +
  ...
  geom_sf(data = acc_iris |>  st_centroid(),
          aes(size= nb_acc_grav), colour="#E84923CC", show.legend = 'point') +
  scale_size(name = "Nombre d'accidents graves",
  ...
  labs(title = "Nombre d'accidents graves de la route à Paris par iris",
...
bks <- unique(round(quantile(100*acc_iris$nb_velo/acc_iris$nb_acc,
                         na.rm=TRUE,
                         probs=seq(0,1,0.1)),1))
...
mf_map(acc_iris, colour = "ivory3", fill = "ivory", add = TRUE)
# Plot symbols with choropleth coloration
mf_map(
  x = acc_iris |>  st_centroid(),
  var = c("nb_acc_grav", "txaccvelo"),
  type = "prop_choro",
  border = "grey50",
  lwd = 0.1,
  leg_pos = c("topright","right"),
  leg_title = c("Nombre d'accidents graves", "Part des accidentés à vélo"),
  breaks = "equal",
  nbreaks = 5,
  inches= 0.2,
  pal = "Reds",
...
mf_layout(
  title = "Nombre d'accidents de la route à Paris par iris par arrondissement à Paris en 2019",
