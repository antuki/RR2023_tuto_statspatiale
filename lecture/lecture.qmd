...
<small>
Il est possible de contrôler finement la jointure (points sur les frontières, ...) avec l'argument `join` qui par défaut est fixé à `st_intersects()`. Voir [ici](https://en.wikipedia.org/wiki/DE-9IM) pour une définition précise des prédicats spatiaux possibles. 
</small>
